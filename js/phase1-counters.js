...
/* World Bank returns both individual countries and aggregate entities such as
   World, income groups and regions. Only records with a real country region
   are admitted to the shared country dataset. */
function latestByCountry(data){const out=new Map();for(const x of data[1]||[]){if(typeof x.value!=='number'||!x.countryiso3code)continue;const code=x.countryiso3code;if(code.length!==3)continue;if(!x.country?.region||x.country.region.id==='NA')continue;const year=Number(String(x.date).slice(0,4));const old=out.get(code);if(!old||year>old.year)out.set(code,{value:x.value,year,name:x.country?.value||code});}return out;}
...
function updateCountryPage(){
 ...
 if(page==='births'||page==='least-births'){const rows=countryRows('births',page==='least-births');const shown=rows.slice(0,50);const perDay=shown.reduce((s,r)=>s+r[2],0),v=perDay*sec/DAY;setHero(page==='least-births'?'Least Births Today':'Births Today',v,'ESTIMATED · '+(perDay/DAY).toFixed(2)+' / SEC');renderRows(shown.map(r=>[r[0],r[1],r[2]*sec/DAY,r[3],r[4],r[5]]));return;}
